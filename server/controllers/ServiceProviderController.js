...
function init() {
  ...
  ipcMain.handle('service-provider:delete', async (event, id) => {
    try {
      ServiceProvider.deleteServiceProvider(id);
      return success(null, '服务商配置已删除');
    } catch (err) {
      console.error('删除服务商配置失败:', err);
      return error(err.message);
    }
  });
  ...
  // 获取服务商支持的声音角色
  ipcMain.handle('service-provider:get-voice-roles', async (event, id) => {
    try {
      const voiceRoles = await ServiceProvider.getVoiceRoles(id);
      return success(voiceRoles);
    } catch (err) {
      console.error('获取声音角色失败:', err);
      return error(err.message);
    }
  });
}

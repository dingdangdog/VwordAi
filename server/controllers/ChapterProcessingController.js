...
function init() {
  ...
  console.log("Chapter processing controller initialized - business logic only");
}
...
function registerLLMProcessingHandlers() {
  ...
  ipcMain.handle('llm:parse-chapter', async (_, chapterId) => {
    try {
      log.info(`[ChapterProcessingController] 解析章节: ${chapterId}`);
      ...
      // 解析章节文本
      const parseResult = await LLMService.parseText(chapter.content, providerType, providerConfig);
      ...
      log.error(`[ChapterProcessingController] 解析章节失败: ${err.message}`, err);
  ...
  ipcMain.handle('llm:parse-chapters-batch', async (_, chapterIds) => {
    try {
      log.info(`[ChapterProcessingController] 批量解析章节: ${chapterIds.length}个`);
      ...
      log.error(`[ChapterProcessingController] 批量解析章节失败: ${err.message}`, err);
      return error(err.message);
    }
  });
}
...
function registerTTSProcessingHandlers() {
  ...
  ipcMain.handle("tts:synthesize-chapter", async (_, chapterId) => {
    try {
      log.info(`[ChapterProcessingController] 合成章节语音: ${chapterId}`);
      ...
      log.error(`[ChapterProcessingController] 合成章节语音失败: ${err.message}`, err);
      return error(err.message);
    }
  });
  ...
  ipcMain.handle("tts:synthesize-chapters-batch", async (_, chapterIds) => {
    try {
      log.info(`[ChapterProcessingController] 批量合成章节语音: ${chapterIds.length}个`);
      ...
      log.error(`[ChapterProcessingController] 批量合成章节语音失败: ${err.message}`, err);
      return error(err.message);
    }
  });

  // 合成解析后的章节段落语音
  ipcMain.handle("tts:synthesize-parsed-chapter", async (_, chapterId, parsedData) => {
    try {
      log.info(`[ChapterProcessingController] 合成解析章节语音: ${chapterId}`);
      ...
      if (parsedData && parsedData.segments) {
        for (const segment of parsedData.segments) {
          ...
          const segmentResult = await synthesizeSegmentText(segment, chapterId);
      ...
      log.error(`[ChapterProcessingController] 合成解析章节语音失败: ${err.message}`, err);
      return error(err.message);
    }
  });
}

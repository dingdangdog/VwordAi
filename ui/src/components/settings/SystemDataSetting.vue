    ...
    <div class="card mb-6">
      ...
      <div class="grid sm:grid-cols-2 gap-6">
        <div class="card bg-gray-50 dark:bg-gray-800 p-6">
          ...
          >
            <ArrowDownTrayIcon v-if="!isExporting" class="h-5 w-5 mr-2" />
        ...
        <div class="card bg-gray-50 dark:bg-gray-800 p-6">
          ...
          <button
            ...
            class="btn btn-primary w-full mt-2"
          >
            <ArrowUpTrayIcon v-if="!isImporting" class="h-5 w-5 mr-2" />
            <div v-else class="animate-spin h-5 w-5 border-2 border-white border-t-transparent rounded-full mr-2"></div>
...
import { settingsApi, projectApi, chapterApi, serviceProviderApi } from '@/utils/api';
...
async function handleFileImport(event: Event) {
  ...
  try {
    ...
    // 确认导入
    if (!confirm('导入将替换当前系统中的所有数据。这个操作无法撤销。确定要继续吗？')) {
      input.value = '';
...
function confirmReset() {
  if (confirm('警告：这将删除所有系统数据，包括项目、章节、设置和服务商配置。这个操作无法撤销。确定要继续吗？')) {
    resetSystemData();
  }
}

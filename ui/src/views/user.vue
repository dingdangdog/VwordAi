  ...
  <div class="h-full flex justify-between">
    <div class="w-40 h-full bg-black/20 flex flex-col">
      <a
        ...
        :class="selectedMenu == menu.path ? 'bg-gray-800' : ''"
        @click="selectedMenu = menu.path"
      >
        {{ menu.name }}
      </a>
    </div>
    <div class="flex-1 h-full overflow-y-auto">
      <UserInfo v-show="selectedMenu === 'info'" />
      <UserOrder v-show="selectedMenu === 'order'" />
      <UserProject v-show="selectedMenu === 'project'" />
      <UserUsed v-show="selectedMenu === 'used'" />
    </div>
  </div>
</template>

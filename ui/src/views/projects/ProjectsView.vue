...
async function handleFileImport(event: Event) {
  ...
  try {
    ...
    if (!projectData.id || !projectData.title || !Array.isArray(projectData.chapters)) {
      throw new Error('无效的项目导入文件格式');
    }

    // Check if project ID already exists
    const existingProject = projectsStore.projects.find(p => p.id === projectData.id);
    if (existingProject) {
      // Ask for confirmation to overwrite or create new
      if (!confirm(`项目 "${projectData.title}" 已存在，是否更新现有项目？点击"取消"将创建新项目。`)) {
    ...
    toast.error(`导入失败: ${error instanceof Error ? error.message : '无效的文件格式'}`);
    // Reset file input
    input.value = '';
  }
}
...
async function createNewProjectFromImport(projectData: any, generateNewId: boolean = true) {
  try {
    // Create new project
...
async function updateExistingProject(projectData: any) {
  try {
    ...
    const existingChapters = projectsStore.chapters.filter(c => c.projectId === projectData.id);

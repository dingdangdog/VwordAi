...
export const useBiliLiveStore = defineStore("biliLive", () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (response.data.alibaba) {
          alibabaConfig.value = { ...alibabaConfig.value, ...response.data.alibaba };
        }
        
        if (response.data.sovits) {
          sovitsConfig.value = { ...sovitsConfig.value, ...response.data.sovits };
        }
      } else {
  ...
  async function saveBiliConfig(configData: Partial<BiliLiveConfig>) {
    ...
    try {
      ...
      const sessdataLength = updatedConfig.SESSDATA ? updatedConfig.SESSDATA.length : 0;
      console.log(`将要保存的SESSDATA长度: ${sessdataLength}`);
      
      const response = await biliLiveService.saveBiliConfig(updatedConfig as BiliLiveConfig);
   ...
   * @param content 消息内容
   */
  function addSystemMessage(type: 'info' | 'warning' | 'error' | 'notice', content: string) {
   ...
   * @param roomId 房间ID
   */
  function setConnectionStatus(connected: boolean, roomId: string | number | null) {
